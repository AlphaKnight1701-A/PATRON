...
import { Building2, Camera, Laptop, Settings, UserCircle } from "lucide-react";
...
function makeResponse(agent: Agent, query: string) {
  ...
  return `I routed “${cleanQuery}” to ${agent}. The demo orchestrator found ${resultSets[agent].length} high-confidence options or verification findings. Review the result before taking action.`;
}
...
export default function Home() {
  ...
  return (
    <main className="app-shell">
      <header className="topbar"><div className="brand-lockup"><span className="brand-mark">P</span><span>patron</span><span className="brand-divider" /> <span className="workspace-label">SUPPLY CHAIN DESK</span></div><div className="topbar-actions"><span className="demo-pill"><span className="status-dot" /> DEMO MODE</span><div className="account-wrap"><button className="avatar" aria-label="Open account menu" aria-expanded={accountOpen} onClick={() => setAccountOpen((open) => !open)}>JD</button>{accountOpen && <div className="account-menu"><div className="account-name"><UserCircle size={18} /> <span><strong>Jordan Diaz</strong><small>Acme IT Services</small></span></div><button onClick={() => { setAccountOpen(false); showModal("Account settings", "Identity, organization membership, and permissions will be connected in the account milestone.", "Identity connector pending"); }}><UserCircle size={15} /> Account settings</button><button onClick={() => { setAccountOpen(false); showModal("Preferences", "Language, notification, marketplace, and privacy preferences will be configurable here.", "Preferences preview"); }}><Settings size={15} /> Preferences</button></div>}</div></div></header>
      <div className="workspace">
        ...
        <section className="main-content"><div className="page-intro"><div><div className="overline">WEDNESDAY, SEPTEMBER 16, 2026</div><h1>Good morning, Jordan.</h1><p>Coordinate your next move across the supply chain.</p></div><button className="outline-button" onClick={() => alert("Report generation is available in the live connector build.")}>Export activity <span>↓</span></button></div>
          ...
          <section className="bottom-grid"><div className="ledger-panel"><div className="section-heading compact"><div><div className="overline">VISUAL SUPPLY CHAIN LEDGER</div><h2>Supply verification</h2></div><button className="text-button">Open ledger <span>↗</span></button></div><div className="ledger-item"><span className="ledger-icon coral">▧</span><div><strong>Incoming laptop batch verified</strong><span>Photo evidence linked to PO-1048</span></div><time>12 min ago</time></div><div className="ledger-item"><span className="ledger-icon blue">✓</span><div><strong>Supplier diversity record updated</strong><span>Ownership proof • review ready</span></div><time>Yesterday</time></div></div><div className="impact-panel"><div className="overline">CIRCULAR ECONOMY SNAPSHOT</div><h2>Make every purchase count.</h2><div className="impact-number">284<span> kg</span></div><p>estimated e-waste avoided this quarter</p><div className="impact-bar"><span /></div><div className="impact-foot"><span>+18% vs last quarter</span><span>Q3 2026</span></div></div></section>
          ...
          <section className="quick-actions"><div><div className="overline">NEXT MOVES</div><h2>Keep the work moving.</h2></div><div className="quick-action-list"><button onClick={() => showModal("Procurement queue", "Three demo requests are staged for human review. Live queue persistence and approvals are part of the next backend milestone.", "3 requests staged")}>Open queue <span>→</span></button><button onClick={() => showModal("Compliance reports", "Report generation is scoped for stored sourcing evidence, supplier diversity records, and reviewed sustainability calculations.", "Human approval required")}>Build a report <span>→</span></button><button onClick={() => showModal("Profile and team", "Profile, organization settings, and team permissions will be connected in the identity milestone.", "Identity connector pending")}>Manage team <span>→</span></button></div></section>
        </section>
      </div>
